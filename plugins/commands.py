...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ Jion Gʀᴏᴜᴘ 1', url="https://t.me/+QX8_YtJsbfFkNDc1"),
                    InlineKeyboardButton('⌬ Join Gʀᴏᴜᴘ 2', url='https://t.me/+fWFg2oxW7rYwODY1')
                ],[
                  InlineKeyboardButton('ɢᴇᴛ ᴀᴅs ғʀᴇᴇ ғɪʟᴇs', callback_data='source')
                ],[
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ Jion Gʀᴏᴜᴘ 1', url="https://t.me/OUTFLiX"),
                    InlineKeyboardButton('⌬ Join Gʀᴏᴜᴘ 2', url='https://t.me/OUTFLiX')
                ],[
                  InlineKeyboardButton('ɢᴇᴛ ᴀᴅs ғʀᴇᴇ ғɪʟᴇs', callback_data='source')
                ],[
                    InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ✇', url=CHNL_LNK)
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/OUTFLiX")
                            ],
                            [
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"{'@Animedekho123 '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Animedekho123 ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@OUTFLiX 🦋' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@OUTFLiX 🦋--> {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
        ...
        reply_markup=InlineKeyboardMarkup(
            [
             [
              ...
              InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="https://t.me/rohan89274")
             ],
             [
              InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @LazyDeveloperr
             ]
